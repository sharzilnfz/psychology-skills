...
export function resolveStateFile(sessionDir) {
  ...
  const stateJson = path.join(sessionDir, 'state.json');
  if (fs.existsSync(stateJson)) return stateJson;
  const stateFile = fs
    .readdirSync(sessionDir)
    .find((f) => /^state\..+\.json$/.test(f));
  return stateFile ? path.join(sessionDir, stateFile) : null;
}
...
export function compactSession(sessionDir, { before = INSIGHT_OLDER_THAN_SESSIONS, keepRecent = true } = {}) {
  ...
  report.before.stateBytes = stateFile && fs.existsSync(stateFile) ? fs.statSync(stateFile).size : 0;
  report.before.profileBytes = profileFile && fs.existsSync(profileFile) ? fs.statSync(profileFile).size : 0;

  // --- 2. Key insights: archive the old, keep recent + open-thread references ---
  ...
  // Keep only the 5 most recent insights among those retained (compact mode).
  ...
  // --- 3. Open threads: archive resolved / stale ones ---
  ...
  // --- 4. Commitments: keep pending + last 3 resolved ---
  ...
  // --- 5. Deduplicate recurringPatterns in profile.json ---
  ...
  // --- 6. state-summary.json digest ---
  const activePatternIds = dedupedPatterns.filter((p) => p.status === 'active').map((p) => p.id);
  const topInsights = keepInsights.slice(-3).map(insightText);
  const summary = {
    niche: state.niche ?? profile.niche ?? null,
    sessionNumber,
    currentPhase: state.currentPhase ?? null,
    nextPhase: state.nextPhase ?? null,
    activeThreadCount: keepThreads.length,
    pendingCommitments: pending.map(commitmentText),
    topInsights,
    activePatternIds,
    riskLevel: state.riskLevel ?? profile.riskFlags?.currentRiskLevel ?? 0,
    stageOfChange: state.stageOfChange ?? 'unknown',
    lastCompacted: today,
    lastUpdated: state.lastUpdated ?? profile.lastUpdated ?? null,
  ...
  // --- Update active state / profile (originals never deleted) ---
  ...
  if (stateFile) writeJson(stateFile, state);
  if (profileFile && patternsChanged) writeJson(profileFile, profile);

  report.after.stateBytes = stateFile && fs.existsSync(stateFile) ? fs.statSync(stateFile).size : 0;
  ...
  // --- 7. Before/after byte counts and token savings ---
