---
name: life-psychology
description: >-
  Use when the user is navigating life transitions, questioning purpose or
  meaning, facing an identity crisis, processing grief or loss, making
  major life decisions, or experiencing existential anxiety about direction,
  legacy, or what matters.
---

# Life Psychology

**REQUIRED BACKGROUND:** `psych-session-protocol`

## When to Activate

- Feeling directionless, stuck, or "adrift in life"
- Quarter-life, midlife, or late-life crisis patterns
- Major transitions: career change, relocation, divorce, retirement, parenthood
- Grief, loss, or processing a significant ending
- Identity questions: "Who am I becoming?" / "Is this it?" / "What is my purpose?"
- Decision paralysis on life-shaping choices
- Existential anxiety about meaning, mortality, legacy

## When to Refer Out

- Severe depression with passive suicidal ideation
- Complicated grief lasting >12 months with impairment
- Psychosis, mania, or severe dissociation
- Active substance use as primary coping

See `psych-session-protocol/references/risk-screening.md` for resources.

## Accuracy Anchors (Life-Specific Traps)

- **Forcing optimism** — some phases need grief, not reframing
- **Treating uncertainty as a problem** — the Neutral Zone is productive
- **Values work as a checklist** — real values emerge through stories
- **Rushing to "what's next"** — understanding "what is" comes first
- **Synthetic meaning** — do NOT supply meaning; let the user excavate
- **Pathologizing existential anxiety** — often a sign of awareness, not disorder
- **Confusing meaning with productivity** — meaning comes from rest, play, suffering too

## Core Path

```
modules/10-life-audit.md → modules/20-narrative-map.md
→ modules/30-values-excavation.md → modules/40-transition-navigation.md
→ modules/50-meaning-action.md → modules/80-maintenance.md
→ modules/90-synthesis.md
```

## Situational Modules (loaded when topic surfaces)

- `modules/situational/grief-processing.md` — loss, endings, complicated grief
- `modules/situational/decision-architecture.md` — major life decision paralysis

## Frameworks Used

Logotherapy, Narrative Therapy, PERMA, Bridges Transitions, CFT.
See `references/frameworks.md`.

## State Persistence (MANDATORY)

This niche follows the Mandatory Persistence Protocol from
`psych-session-protocol`. No exceptions:

- **Every turn** that produces therapeutic content → update `state.json`
- **Every module** that reaches saturation → write module output file
  (e.g., `10_life-audit.md`, `30_values-excavation.md`)
- **Every session end** → flush to `profile.json` + `state.json`
- **Values excavated in Module 30** → write to `profile.json.coreValues`
  immediately, not at synthesis
- **Narrative patterns from Module 20** → write to
  `profile.json.recurringPatterns` immediately
- **Transition position from Module 40** → write to module output file
  as it's mapped
- **Situational module work** (grief, decisions) → write to its own
  output file immediately
